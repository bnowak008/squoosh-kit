...
import { isWorker, hasImageData } from '@squoosh-lite/core';
import type {
  WorkerRequest,
  WorkerResponse,
} from '@squoosh-lite/core';
import type { ImageInput } from './index.ts';
import type { ResizeOptions } from './index.ts';
...
async function ensureResizeInitialized(): Promise<void> {
  ...
  try {
    ...
    // Path is relative to this file (packages/resize/src/)
    const modulePath = new URL(
      '../../core/wasm/resize/squoosh_resize.js',
      import.meta.url
    ).href;
    ...
    const wasmPath = new URL(
      '../../core/wasm/resize/squoosh_resize_bg.wasm',
      import.meta.url
    ).href;
    ...
    throw new Error(
      `Failed to load Resize module: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
      ...
      if (request.type === 'resize:run') {
        ...
        const buffer = (
          result.data instanceof Uint8Array
            ? result.data.buffer
            : (result.data as Uint8ClampedArray).buffer
        ) as ArrayBuffer;
