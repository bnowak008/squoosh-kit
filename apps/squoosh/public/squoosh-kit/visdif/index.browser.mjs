...
// src/visdif.worker.ts
var exports_visdif_worker = {};
__export(exports_visdif_worker, {
  visdifCompareClient: () => visdifCompareClient
});
async function loadVisDifModule() {
  if (cachedModule)
    return cachedModule;
  ...
  for (const jsPath of jsPaths) {
    try {
      moduleFactory = (await import(jsPath)).default;
      break;
    } catch (error) {
  ...
  const wasmPaths = isSource ? ["../wasm/visdif/visdif.wasm", "./wasm/visdif/visdif.wasm"] : ["./wasm/visdif/visdif.wasm", "../wasm/visdif/visdif.wasm"];
  let wasmBinary = null;
...
async function visdifCompareClient(image1, image2, signal) {
  if (signal?.aborted) {
    ...
    self.onmessage = async (event) => {
      ...
      try {
        if (request.type === "visdif:compare") {
          const result = await visdifCompareClient(request.payload.image1, request.payload.image2);
          response.ok = true;
...
class VisDifClientBridge {
  async compare(image1, image2, signal) {
    const module = await Promise.resolve().then(() => (init_visdif_worker(), exports_visdif_worker));
    const visdifCompareClient2 = module.visdifCompareClient;
    return visdifCompareClient2(image1, image2, signal);
  }
  async terminate() {}
}

class VisDifWorkerBridge {
  worker = null;
  workerReady = null;
  options;
  constructor(options) {
...
async function compare(image1, image2, signal) {
  if (!globalBridge) {
    globalBridge = createBridge("worker");
  }
  return globalBridge.compare(image1, image2, signal);
}
function createVisDiff(mode = "worker", options) {
  const bridge = createBridge(mode, options);
  return Object.assign((image1, image2, signal) => {
...
//# debugId=4604BF439F7278B164756E2164756E21
