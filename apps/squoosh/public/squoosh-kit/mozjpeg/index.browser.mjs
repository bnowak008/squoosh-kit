...
class MozjpegClientBridge {
  async encode(image, options, signal) {
    const module = await Promise.resolve().then(() => (init_mozjpeg_worker(), exports_mozjpeg_worker));
    const mozjpegEncodeClient2 = module.mozjpegEncodeClient;
    return mozjpegEncodeClient2(image, options, signal);
  }
  async decode(data, signal) {
    const module = await Promise.resolve().then(() => (init_mozjpeg_worker(), exports_mozjpeg_worker));
    const mozjpegDecodeClient2 = module.mozjpegDecodeClient;
...
//# debugId=4DCCFE07F0D5902F64756E2164756E21
