...
class RotateClientBridge {
  async rotate(image, options, signal) {
    const module = await Promise.resolve().then(() => (init_rotate_worker(), exports_rotate_worker));
    const rotateClient2 = module.rotateClient;
    return rotateClient2(image, options, signal);
  }
  async terminate() {}
}

class RotateWorkerBridge {
  worker = null;
  workerReady = null;
  options;
  constructor(options) {
...
async function rotate(image, options, signal) {
  if (!globalBridge) {
    globalBridge = createBridge("worker");
  }
  return globalBridge.rotate(image, options, signal);
}
function createRotator(mode = "worker", options) {
...
//# debugId=80D3A7E0F5356F0264756E2164756E21
