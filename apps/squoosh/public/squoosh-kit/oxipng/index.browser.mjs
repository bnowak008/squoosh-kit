...
// src/oxipng.worker.ts
var exports_oxipng_worker = {};
__export(exports_oxipng_worker, {
  oxipngOptimizeClient: () => oxipngOptimizeClient
});
async function initOxiPNG() {
  ...
  initPromise = (async () => {
    try {
      ...
      if (!wasmBuffer) {
        throw lastError || new Error("Could not load OxiPNG WASM binary from any path");
      }
      ...
      for (const jsPath of jsPaths) {
        try {
          oxipngModule = await import(jsPath);
          break;
        } catch (error) {
      ...
      if (!oxipngModule) {
        throw jsLastError || new Error("Could not load OxiPNG JS module from any path");
      }
      await oxipngModule.default(wasmBuffer);
      optimiseFn = oxipngModule.optimise;
    } catch (error) {
      initPromise = null;
      throw new Error(`Failed to initialize OxiPNG WASM module: ${error instanceof Error ? error.message : String(error)}`, { cause: error });
    }
  })();
  return initPromise;
}
async function oxipngOptimizeClient(image, options, signal) {
  if (signal?.aborted) {
  ...
  const clampedData = data instanceof Uint8ClampedArray ? data : new Uint8ClampedArray(data.buffer, data.byteOffset, data.length);
...
var optimiseFn = null, initPromise = null;
var init_oxipng_worker = __esm(() => {
  init_src();
...
class OxipngClientBridge {
  async optimize(image, options, signal) {
    const module = await Promise.resolve().then(() => (init_oxipng_worker(), exports_oxipng_worker));
    const oxipngOptimizeClient2 = module.oxipngOptimizeClient;
    return oxipngOptimizeClient2(image, options, signal);
  }
  async terminate() {}
}

class OxipngWorkerBridge {
  worker = null;
  workerReady = null;
  options;
  constructor(options) {
...
//# debugId=9FE70606C9282ED564756E2164756E21
