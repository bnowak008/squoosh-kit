...
import AmbientBlobs from './components/blobs/AmbientBlobs';
import HeroBlobs from './components/blobs/HeroBlobs';
...
function WaveSeparator() {
  ...
  useEffect(() => {
    ...
    el.setAttribute('wave-height', '200px');
    el.setAttribute('responsive-factor', '.5');
    el.setAttribute('opacity-range', '.33, 1');
    el.setAttribute('animation-style', 'bouncy');
  }, []);
    ...
    >
      ...
      >
        ...
        <div className="absolute inset-0 flex min-w-0 flex-col md:flex-row z-10">
          {/* Logo row */}
          <div
            className={`flex items-center justify-center mb-2 lg:mb-5 pt-2 ${showingEditor && isMobile ? '' : 'hidden'}`}
          >
            <Logo />
          </div>
          ...
          >
            <InfoPanel codecId={state.codecId} />
          </div>
