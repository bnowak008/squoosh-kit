...
export default function ResizePanel({
  ...
  function handleWidth(raw: string) {
    ...
    onChange({ ...options, width: val, height: locked ? Math.round(val * ratio) : h });
  }

  function handleHeight(raw: string) {
    const val = Math.max(1, parseInt(raw, 10) || 1);
    onChange({ ...options, height: val, width: locked ? Math.round(val / ratio) : w });
  }
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex items-center justify-between">
        <span className="text-xs font-medium text-white/70 uppercase tracking-wide">Resize</span>
        <button
      ...
      {enabled && (
        <>
          ...
          <select
            ...
            onChange={(e) => onChange({ ...options, method: e.target.value as ResizeOptions['method'] })}
