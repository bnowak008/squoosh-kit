...
export type ImageInput = ImageData | { data: Uint8Array; width: number; height: number };
...
class ClientBridge implements WorkerBridge {
  async webpEncode(
    ...
    const { webpEncodeClient } = await import('../features/webp/webp.worker.ts');
  ...
  async resize(
    ...
    // Dynamically import the client implementation
    const { resizeClient } = await import('../features/resize/resize.worker.ts');
